...
namespace OSK.UI
{
    ...
    public class UITransition : MonoBehaviour
    {
        ...
        {
            var image = gameObject.GetOrAddComponent<UnityEngine.UI.Image>();
        ...
        public void OpenTrans(Action onComplete)
        {
            ...
            switch (_openingTweenSettings.transition)
            {
                case TransitionType.Fade:
                    ...
                    {
                        Debug.Log("_canvasGroup not add to component => " + gameObject.name);
        ...
        private void OnCompletedTween(TweenSettings tweenSettings, Tween tween, Action onComplete, bool isOpen)
        {
            if (tweenSettings.transition == TransitionType.Animation)
            {
                if (tweenSettings.animationClip != null)
                {
                   ...
                   StartCoroutine(WaitForCompleted(clipLength, () =>
                    {
                        onComplete?.Invoke();
                    }));
                }
        ...
        private IEnumerator WaitForCompleted(float clipLength, Action onComplete)
        {
            if (clipLength <= 0)
            {
                onComplete?.Invoke();
